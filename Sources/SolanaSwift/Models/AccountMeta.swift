...
public struct AccountMeta: Equatable, Codable, CustomDebugStringConvertible {
    ...
    enum CodingKeys: String, CodingKey {
        case name, isSigner, isMut
    }

    public init(from decoder: Decoder) throws {
        let values = try decoder.container(keyedBy: CodingKeys.self)
        publicKey = try PublicKey(string: values.decode(String.self, forKey: .name))
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(publicKey.base58EncodedString, forKey: .name)
